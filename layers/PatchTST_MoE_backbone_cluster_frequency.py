...
class PatchTST_MoE_cluster_frequency(nn.Module):
    def __init__(self, bs: int, c_in: int, c_out: int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        # Backbone
        self.backbone = TSTiEncoder(c_in, patch_num=patch_num_in, patch_len=patch_len, max_seq_len=max_seq_len,
        ...
        # Cluster
        self.cluster = EDESC(d_model=d_model, n_clusters=F_num_expert, eta=eta, c_out=c_out, bs=bs,
                             patch_len=patch_len, stride=stride)

        # MoE
        self.SaprseMoE = SparseMoE(n_embed=c_in * d_model, num_experts=F_num_expert, top_k=F_top_k)
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        # model
        # z = torch.reshape(z, (z.shape[0] * z.shape[3] * z.shape[1], z.shape[2]))  # z: [bs * patch_num_out x nvars * d_model]
...
class TSTEncoder(nn.Module):
    ...
    def forward(self, src: Tensor, key_padding_mask: Optional[Tensor] = None, attn_mask: Optional[Tensor] = None):
        ...
        if self.res_attention:
            for mod in self.layers: output = mod(output, prev=scores, key_padding_mask=key_padding_mask,
                                                         attn_mask=attn_mask)
            return output
        else:
